...
export const heartbeatWorld = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (worldStatus.status === 'stoppedByHumanVictory') {
      console.debug(`World ${worldStatus.worldId} is stopped due to human victory, not restarting.`);
    }
...
export const joinWorld = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingPlayer) {
      throw new Error(`Already joined as ${existingPlayer.id}`);
    }
